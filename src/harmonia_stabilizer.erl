    ...
    {NodeName, NodeVector} = 
        case Succ of
                ...
                case hm_misc:get_first_alive_entry(tl(State#state.succlist)) of
                    ...
                    {NewSucc, NewSuccVector} -> 
                        ?debug_p("stabilize_loop:NewSucc:[~p] NewSuccVector:[~p].~n", RegName, 
                               [NewSucc, NewSuccVector]),
                        make_succ_list({NewSucc, NewSuccVector}, harmonia:name(RegName)),
            ...
            {ok, {SuccName, SuccVector}} ->
                ...
                make_succ_list({SuccName, SuccVector}, harmonia:name(RegName)),
                {SuccName, SuccVector}
        end,
...
fixfinger_loop(timeout, RegName) ->
    ...
    %    inquiry to it
    NewSucc = gen_server:call(harmonia:name(RegName), {find_successor, NodeVector, Current}),
    ...
    gen_server:cast(harmonia:name(RegName), {fix_finger, Current, NewSucc, State#state.finger}),
