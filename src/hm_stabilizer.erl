...
stabilize_loop(timeout, RegName) ->
    ...
    gen_server:cast(hm_router:name(RegName), check_pred),

    % node is checked to be alive inside get_successor
    {ok, State} = gen_server:call(hm_router:name(RegName), state_info),
...
check_new_successor(SuccName, RegName) ->
    PredOfSucc = gen_server:call(SuccName, get_predecessor),
    gen_server:cast(hm_router:name(RegName), {stabilize, PredOfSucc}).


make_succ_list({SuccName, SuccVector}, MyNodeName) ->
    SuccList = [{SuccName, SuccVector} |
                gen_server:call(SuccName, copy_succlist)],
    NewSuccList = 
    ...
    gen_server:cast(MyNodeName, {set_succlist, NewSuccList}).
...
fixfinger_loop(timeout, RegName) ->
    ...
    %    inquiry to it
    NewSucc = gen_server:call(hm_router:name(RegName), {find_successor, NodeVector, Current}),
    ...
    gen_server:cast(hm_router:name(RegName), {fix_finger, Current, NewSucc, State#state.finger}),
...
name(Name) -> list_to_atom("hm_stabilizer" ++ atom_to_list(Name)).
